...
//Code Here
var evenNumbers = mixedNumbers.filter(function(element, index) {
  return (element % 2 === 0);
});
...
//Code Here
let postTaxPrices  = prices.map(function(el, i) {
  return (el * 1.07);
});
...
let ordersTotal = orders.map(function(price, tax) {
  return price + tax;
})
...
let bobsTotal = purchases.reduce(function(accum, currentVal, currentIndex) {
  if (purchases[currentIndex].owner == "Bob") {
